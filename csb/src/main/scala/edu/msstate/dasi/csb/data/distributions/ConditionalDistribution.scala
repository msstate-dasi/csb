...
protected class ConditionalDistribution[Value: ClassTag, Cond: ClassTag](data: RDD[(Value, Cond)]) extends Serializable {
  ...
  private val distributions = {
    ...
    val distributions = occurrences.map { case ((value, cond), count) => (cond, (value, count)) }
      ...
      .map {
        case (cond, ((value, count), sum)) => (cond, Array((value, count / sum.toDouble))) // Normalize to obtain probabilities
      }
      .reduceByKey(_ ++ _)
      .map {
        case (cond, array) => (cond, new Distribution(array.sortBy(_._2)(Ordering[Double].reverse))) // Descending order to maximize sampling performance
      }
